...
def mock_synthesize():
    with patch("reporters.weekly.synthesize_section", return_value="Narration de test.") as m:
        yield m
...
def test_build_weekly_returns_report_output():
    """Test 1: build_weekly_report returns a ReportOutput."""
    r = build_weekly_report({}, _make_config())
    assert isinstance(r, ReportOutput)


def test_plain_text_contains_executive_summary_heading():
    """Test 2: plain_text contains '## Executive Summary' (Markdown section heading)."""
    r = build_weekly_report({}, _make_config())
    assert "## Executive Summary" in r.plain_text
...
def test_html_body_etf_chart_embedded_when_returned():
    ...
    with patch("reporters.weekly.generate_etf_chart", return_value="abc123"):
        r = build_weekly_report({}, _make_config())
...
def test_html_body_pea_html_embedded_when_returned():
    ...
    with patch("reporters.weekly.generate_pea_table", return_value="<table>pea</table>"):
        r = build_weekly_report({}, _make_config())
...
def test_total_degradation_returns_report_output():
    """Test 10: When data={}, build_weekly_report still returns a ReportOutput (never raises)."""
    r = build_weekly_report({}, _make_config())
    assert isinstance(r, ReportOutput)
...
def test_plain_text_has_section_indisponible_on_full_failure():
    ...
    with patch("reporters.weekly._executive_summary", side_effect=RuntimeError("boom")), \
         patch("reporters.weekly._macro_watch", side_effect=RuntimeError("boom")), \
         patch("reporters.weekly._etf_performance", side_effect=RuntimeError("boom")), \
         patch("reporters.weekly._crypto_recap", side_effect=RuntimeError("boom")), \
         patch("reporters.weekly._pea_wrap", side_effect=RuntimeError("boom")), \
         patch("reporters.weekly._news_digest", side_effect=RuntimeError("boom")), \
