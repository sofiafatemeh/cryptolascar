      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        {!report ? (
          <div className="text-center py-24">
            ...
            <p className="text-blanc-linen/30 text-sm">Aucun rapport disponible.</p>
            <p className="text-blanc-linen/20 text-xs mt-1">
              Le VPS n&apos;a pas encore envoyé de données.
            </p>
          </div>
        ) : (
        ...
        {archives.length > 1 && (
          <div className="mt-10 pt-6 border-t border-or-sale/15">
            <p className="text-xs uppercase tracking-widest text-blanc-linen/20 mb-3">
              Archives
            </p>
            <div className="flex flex-wrap gap-2">
              {archives.slice(1, 14).map(r => (
                <Link
                  key={r.date}
                  href={`/dashboard/report?type=daily&date=${r.date}`}
